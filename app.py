...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Ichimoku
    ichi = ta.ichimoku(out["high"], out["low"], out["close"], tenkan=9, kijun=26, senkou=52)
    if ichi is not None and len(ichi) == 2:
        span = ichi[0]
        out = pd.concat([out, span], axis=1)
        rename_map = {}
        for c in span.columns:
            cl = str(c).lower()
            if "its" in cl or "tenkan" in cl:
                rename_map[c] = "tenkan"
            elif "iks" in cl or "kijun" in cl:
                rename_map[c] = "kijun"
            elif "isa" in cl or "span_a" in cl:
                rename_map[c] = "span_a"
            elif "isb" in cl or "span_b" in cl:
                rename_map[c] = "span_b"
    ...
    try:
        jaw = ta.rma(median, length=13)
        teeth = ta.rma(median, length=8)
        lips = ta.rma(median, length=5)
    except Exception:
        jaw = ta.sma(median, length=13)
        teeth = ta.sma(median, length=8)
        lips = ta.sma(median, length=5)
    out["jaw"] = jaw.shift(8)
    out["teeth"] = teeth.shift(5)
    out["lips"] = lips.shift(3)
    return out
